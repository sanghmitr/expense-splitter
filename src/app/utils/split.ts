export class CalExpense {
    person1: string;
    person2: string;
    amount: number;

  constructor(p1 : string, p2 : string, val : number) {
    this.person1 = p1;
    this.person2 = p2;
    this.amount = val;
  }
...
function upheapify(heap : any, idx : number) {
...
function downheapify(heap : any, idx : number) {
  var lc = 2 * idx + 1;
  var rc = 2 * idx + 2;
...
function pop_heap(heap : any) : any {
...
// transactions = [{person1: 'sanghmitr', person2: 'ishan', amount: 100}]
export function splitwise(transactions: any) {
  ...
  let positive : any = []; // heap of credit holders
  let negative : any = []; // heap of debit holders
  ...
  let result : CalExpense[] = []; // array of expense objects
